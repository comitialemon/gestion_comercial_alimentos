...
class InformeSucursalEntreFechasController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        // Obtener fechas disponibles para los selects
        ...
        return Inertia::render('Gestion/Reportes/ControlInterno/InformeSucursalEntreFechas', [
            'fechas' => $fechas,
        ]);
    }
    ...
    public function exportar(Request $request)
    {
        ...
        $fechaInicialMostrar = date('d-m-Y', strtotime($fechaInicial));
        $fechaFinalMostrar = date('d-m-Y', strtotime($fechaFinal));
        
        // Obtener IDs de fechas para inventario
        $idFechaInicial = DB::connection('mysql_gestion_comercial_alimentos')
            ->table('todos_fecha')
            ->where('Fecha', $fechaInicial)
            ->value('IdFecha');
        
        $idFechaFinal = DB::connection('mysql_gestion_comercial_alimentos')
            ->table('todos_fecha')
        ...
        $worksheet->setCellValue('A4', 'Del: ' . $fechaInicialMostrar . ' Al: ' . $fechaFinalMostrar);
        ...
        foreach ($sucursales as $suc) {
            ...
            $totalVentas = DB::connection('mysql_gestion_comercial_alimentos')
                ...
                ->where('IdCliente', $clienteId)
                ->where('IdClienteSucursal', $idSucursal)
                ->whereBetween('FechaVenta', [$fechaInicial, $fechaFinal])
                ->sum('ImporteVenta');
        ...
        foreach ($conceptos as $concepto) {
            ...
            foreach ($sucursales as $suc) {
                ...
                $totalBolivianos = DB::connection('mysql_gestion_comercial_alimentos')
                    ...
                    ->whereBetween('impuestos_ventas.FechaVenta', [$fechaInicial, $fechaFinal])
        ...
        foreach ($productosVendidos as $producto) {
            ...
            $worksheet->setCellValue('B' . $i, $tipoVenta . ' - ' . $this->toExcelString($producto->Detalle));
            $worksheet->setCellValue('C' . $i, (float) $producto->TotalUnidades);
            $i++;
        }
        ...
        $inventario = DB::connection('mysql_gestion_comercial_alimentos')
            ...
            ->where('impuestos_ventas.IdCliente', $clienteId)
            ->where('impuestos_ventas.IdClienteSucursal', $sucursalId)
            ->whereBetween('inventario_propiamente.IdFecha', [$idFechaInicial, $idFechaFinal])
        ...
        foreach ($inventario as $item) {
            $worksheet->setCellValue('B' . $i, $this->toExcelString($item->Descripcion));
            $worksheet->setCellValue('C' . $i, (float) $item->TotalUnidades);
            $i++;
        }
        ...
        // Wrap text
        $columnaArrayAux = $columnaArray - 1;
        $letraAuxiliar = $columnas[$columnaArrayAux];
        $worksheet->getStyle('C8:' . $letraAuxiliar . '8')->getAlignment()->setWrapText(true);
        ...
        // DESCARGAR
        // ==========================================
        $nombreArchivo = 'AnalisisSucursalEntreFechas_' . $fechaInicialMostrar . '_al_' . $fechaFinalMostrar . '.xls';
