...
use App\Models\Operacion\FichaTecnica;
use App\Models\Operacion\FichaTecnicaDetalle;
...
class FichaTecnicaController extends Controller
{
    ...
    /**
     * Obtener el ID del estado "Insumos" para el cliente logueado
     */
    public function getEstadoInsumos()
    {
        try {
            ...
            if (!$clienteId) {
                return response()->json([
                    'success' => false,
                    'message' => 'No hay cliente en sesión'
                ], 400);
            }
            
            $estado = DB::connection('mysql_gestion_comercial_alimentos')
                ->table('inventario_producto_estado')
                ->where('IdCliente', $clienteId)
                ->where('Estado', 'Insumos')
                ->first();
            ...
            if (!$estado) {
                return response()->json([
                    'success' => false,
                    'message' => 'No se encontró el estado "Insumos" para este cliente'
                ], 404);
            }
            ...
            \Log::error('Error obteniendo estado Insumos: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Error al obtener el estado Insumos'
            ], 500);
        }
    }
}