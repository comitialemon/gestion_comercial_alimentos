...
const getCategoriaId = (categoria) => {
    return categoria ? (categoria.id ?? categoria.id_categoria) : null;
}
...
const aplicarFiltros = (cerrarFiltros = true) => {
    ...
    router.get('/operacion/reportes/inventario-relacion-venta', params, {
        preserveState: true,
        replace: true,
    })
...
const limpiarFiltros = () => {
    ...
    router.get('/operacion/reportes/inventario-relacion-venta', {}, {
        preserveState: true,
        replace: true,
    })
}
...
const getCategoriaClase = (producto) => {
    if (producto.categoria_nombre && producto.categoria_nombre !== 'Sin categoría') {
        ...
        <div class="py-2 px-2 sm:py-3 sm:px-3 lg:py-4 lg:px-6">
            <div class="max-w-full mx-auto">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-3 sm:mb-4">
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                            ...
                            <i class="fas fa-cubes text-primary-600 text-[11px] sm:text-sm"
                               :style="{ color: `var(--color-primary-600)` }"></i>
                        </div>
                        <div class="min-w-0">
                            <h1 class="text-sm sm:text-base font-bold text-gray-800 truncate">Inventario Relación Venta</h1>
                            <p class="text-[9px] sm:text-[10px] text-gray-500 truncate">Productos y su composición</p>
                        </div>
                    </div>
                ...
                <div class="flex flex-col lg:flex-row gap-3 sm:gap-4">
                    ...
                    <!-- TABLA DE PRODUCTOS CON COMPOSICIÓN EN LA CELDA -->
                    <div class="flex-1 min-w-0">
                        <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-primary-50" :style="{ backgroundColor: `var(--color-primary-50)` }">
                                        <tr>
                                            ...
                                            <th class="px-3 py-2 text-left text-[10px] font-semibold" :style="{ color: `var(--color-primary-700)` }">Detalle</th>
                                            <th class="px-3 py-2 text-left text-[10px] font-semibold" :style="{ color: `var(--color-primary-700)` }">Composición</th>
                                        ...
                                        <tr v-for="producto in productos.data" :key="producto.IdDetalleProducto" class="hover:bg-gray-50 transition">
                                            <td class="px-3 py-2">
                                                <span class="px-1.5 py-0.5 text-[9px] rounded-full" :class="getCategoriaClase(producto)">
                                                    <i v-if="producto.categoria_nombre && producto.categoria_nombre !== 'Sin categoría'" class="fas fa-tag mr-1 text-[8px]"></i>
                                            ...
                                            <td class="px-3 py-2 text-[10px] text-gray-600">
                                                <!-- COMPOSICIÓN DENTRO DE LA CELDA -->
                                                <div v-if="producto.tiene_composicion" class="composicion-container">
                                                    <div class="composicion-header text-[9px] font-semibold text-gray-500 border-b border-gray-200 pb-0.5 mb-0.5">
                                                        <span class="inline-block w-2/3">Producto</span>
                                                        <span class="inline-block w-1/3 text-right">Porción</span>
                                                    </div>
                                                    <div v-for="comp in producto.composicion" :key="comp.IdDetalleProductoPorcion" 
                                                         class="composicion-row text-[9px] py-0.5 border-b border-gray-100 last:border-0">
                                                        <span class="inline-block w-2/3 text-gray-700">{{ comp.producto_detalle }}</span>
                                                        <span class="inline-block w-1/3 text-right font-medium" :style="{ color: `var(--color-primary-600)` }">
                                                            {{ Number(comp.Porcion).toFixed(6) }}
                                                        </span>
                                                    </div>
                                                </div>
                                                <span v-else class="text-gray-400 text-xs italic">Sin composición</span>
                                            </td>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="producto in productos.data" :key="producto.IdDetalleProducto" class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-3 py-2 text-[10px] text-gray-600">
                                                <div v-if="producto.tiene_composicion" class="composicion-container">
                                                    <div class="composicion-header text-[8px] font-semibold text-gray-500 border-b border-gray-200 pb-0.5 mb-0.5">
                                                        <span class="inline-block w-2/3">Producto</span>
                                                        <span class="inline-block w-1/3 text-right">Porción</span>
                                                    </div>
                                                    <div v-for="comp in producto.composicion" :key="comp.IdDetalleProductoPorcion" 
                                                         class="composicion-row text-[8px] py-0.5 border-b border-gray-100 last:border-0">
                                                        <span class="inline-block w-2/3 text-gray-700">{{ comp.producto_detalle }}</span>
                                                        <span class="inline-block w-1/3 text-right font-medium" :style="{ color: `var(--color-primary-600)` }">
                                                            {{ Number(comp.Porcion).toFixed(6) }}
                                                        </span>
                                        ...
                                        <tr v-if="!productos.data || productos.data.length === 0">
                                            <td colspan="3" class="px-3 py-8 text-center text-gray-400 text-[11px]">
                            ...
                            <div class="sm:hidden divide-y divide-gray-100">
                                <div v-for="producto in productos.data" :key="producto.IdDetalleProducto" class="p-3 hover:bg-gray-50 transition">
                                    <div class="text-xs font-medium text-gray-800">{{ producto.Detalle }}</div>
                                    ...
                                    <div class="mt-1 text-[9px] text-gray-600 bg-gray-50 rounded p-1.5">
                                        <div v-if="producto.tiene_composicion" class="composicion-container">
                                            <div class="composicion-header text-[8px] font-semibold text-gray-500 border-b border-gray-200 pb-0.5 mb-0.5">
                                                <span class="inline-block w-2/3">Producto</span>
                                                <span class="inline-block w-1/3 text-right">Porción</span>
                                        ...
                                        <span v-else class="text-gray-400 italic">Sin composición</span>
                                    </div>
                                </div>
...
/* Estilos para la composición en la celda */
