...
const reimprimirFactura = (id) => {
    loading.value = true
    try {
        const url = route('gestion.reportes.informe-ventas.reimprimir', { id })
...
onMounted(() => {
    ...
    if (sucursalId.value) {
        ...
        if (sucursal) sucursalBusqueda.value = sucursal.nombre
    }
    
    if (vendedorId.value) {
        const vendedor = props.vendedores?.find(v => v.id === vendedorId.value)
        if (vendedor) vendedorBusqueda.value = vendedor.nombre_completo
    }
    
    if (comisionistaId.value) {
        const comisionista = props.comisionistas?.find(c => c.id === comisionistaId.value)
        if (comisionista) comisionistaBusqueda.value = comisionista.nombre
    }
            ...
            <div class="max-w-full mx-auto">
                ...
                <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-base lg:text-lg font-bold text-gray-800">Informe de Ventas</h1>
                            <p class="text-xs text-gray-500">Facturas válidas agrupadas por sucursal</p>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 mb-4">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-2.5 border-l-2 border-primary-500">
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-base font-bold text-primary-700">{{ estadisticas.total_ventas || 0 }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[9px] text-gray-400">Bs</p>
                    ...
                    <div v-for="suc in estadisticas.por_sucursal" :key="suc.sucursal_id" 
                        class="bg-white rounded-xl shadow-sm p-2.5 border-l-2"
                        :class="suc.sucursal_id == estadisticas.sucursal_seleccionada ? 'border-primary-500 bg-primary-50' : 'border-gray-300'"
                    >
                        <div class="flex justify-between items-center">
                            <div class="min-w-0 flex-1">
                                <p class="text-[9px] text-gray-400 uppercase tracking-wide truncate">
                                    <i class="fas fa-store text-primary-500 mr-0.5 text-[8px]"></i>
                                    {{ suc.sucursal_nombre }}
                                </p>
                                <p class="text-base font-bold text-primary-700">{{ suc.total_ventas || 0 }}</p>
                            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-3 mb-4">
                    <div class="flex flex-wrap items-end gap-2">
                        ...
                        <!-- Sucursal -->
                        <div class="sucursal-autocomplete flex-1 min-w-[140px] max-w-[200px]">
                            ...
                            <div v-if="sucursalId" class="mt-1">
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-primary-50 text-primary-700 truncate max-w-full">
                            ...
                            <div class="relative">
                                ...
                                <div v-if="mostrarVendedores && vendedoresDisponibles.length > 0" 
                                    ...
                                    <div 
                                        ...
                                        :key="ven.id"
                                        @mousedown.prevent="seleccionarVendedor(ven)"
                                        class="px-2.5 py-1.5 cursor-pointer hover:bg-primary-50 text-xs flex justify-between items-center border-b border-gray-100 last:border-0"
                        ...
                        <!-- Comisionista -->
                        <div class="comisionista-autocomplete flex-1 min-w-[140px] max-w-[200px]">
                            <label class="text-[10px] text-gray-500 font-medium block mb-0.5">Comisionista</label>
                            <div class="relative">
                                <input 
                                    type="text"
                                ...
                                <div v-if="mostrarComisionistas && comisionistasDisponibles.length > 0" 
                                    ...
                                    <div 
                                        ...
                                        :key="com.id"
                                        @mousedown.prevent="seleccionarComisionista(com)"
                                        class="px-2.5 py-1.5 cursor-pointer hover:bg-primary-50 text-xs flex justify-between items-center border-b border-gray-100 last:border-0"
                        ...
                        <div class="flex-1 min-w-[160px] max-w-[240px]">
                            <label class="text-[10px] text-gray-500 font-medium block mb-0.5">N° Factura</label>
                            <div class="flex gap-1.5">
                                <input 
                                    v-model="search"
                                    type="text"
                                    placeholder="N°"
                                    class="flex-1 border border-gray-300 rounded-md px-2.5 py-1 text-sm focus:ring-primary-500 focus:border-primary-500 outline-none min-w-0"
                                    @keyup.enter="aplicarFiltros"
                                />
                                
                                <button 
                                    @click="aplicarFiltros"
                                    class="bg-primary-600 hover:bg-primary-700 text-white px-3 py-1 rounded-md text-xs font-medium transition flex-shrink-0 flex items-center justify-center gap-1.5"
                                    title="Buscar"
                                >
                                ...
                                <button 
                                    ...
                                    class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1 rounded-md text-xs font-medium transition flex-shrink-0 flex items-center justify-center gap-1.5"
                    ...
                    <div class="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-200">
                        ...
                        <span class="text-[10px] text-gray-400 ml-auto self-center hidden sm:inline">
                            {{ sucursalesConVentas.length }} sucursal(es)
                        </span>
                    </div>
                </div>
                ...
                <div v-for="grupo in sucursalesConVentas" :key="grupo.id" class="mb-3">
                    ...
                    <div 
                        ...
                        class="flex flex-wrap items-center justify-between gap-2 bg-primary-50 rounded-t-lg px-3 py-2 border border-primary-200 cursor-pointer hover:bg-primary-100 transition"
                    >
                        <div class="flex items-center gap-1.5 min-w-0 flex-1">
                            <i 
                                class="text-primary-600 text-[10px] transition-transform duration-200 flex-shrink-0"
                                :class="sucursalesExpandidas[grupo.id] ? 'fas fa-chevron-down' : 'fas fa-chevron-right'"
                            ></i>
                            
                            <i class="fas fa-store text-primary-600 text-xs flex-shrink-0"></i>
                            <h2 class="font-bold text-primary-800 text-xs truncate">{{ grupo.nombre }}</h2>
                            <span class="text-[9px] text-primary-600 bg-primary-100 px-1.5 py-0.5 rounded-full flex-shrink-0">
                                {{ grupo.ventas.length }}
                            </span>
                        </div>
                        <div class="text-xs font-bold text-primary-700 flex-shrink-0">
                    ...
                    >
                        <div v-if="sucursalesExpandidas[grupo.id]" class="bg-white rounded-b-lg shadow-sm overflow-hidden border border-t-0 border-primary-200">
                            ...
                            <div class="hidden md:block overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">N° Factura</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">Fecha</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">Estado</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">NIT</th>
                                            <th class="px-3 py-1.5 text-right text-[9px] font-medium text-gray-500 uppercase w-24">Importe</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">Comisionista</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">Vendedor</th>
                                            <th class="px-3 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase">Sucursal</th>
                                            <th class="px-3 py-1.5 text-center text-[9px] font-medium text-gray-500 uppercase w-24">Acción</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200">
                                        <tr v-for="venta in grupo.ventas" :key="venta.IdVentas" class="hover:bg-gray-50 transition">
                                            <td class="px-3 py-2 text-xs font-medium text-primary-700">
                                                {{ venta.NumeroFactura || '-' }}
                                            </td>
                                            ...
                                            <td class="px-3 py-2">
                                                <span class="inline-flex px-1.5 py-0.5 text-[9px] font-medium rounded-full bg-emerald-100 text-emerald-700">
                                            ...
                                            <td class="px-3 py-2 text-xs font-bold text-primary-700 text-right">
                                                {{ Number(venta.ImporteVenta).toFixed(2) }}
                                            </td>
                                            <td class="px-3 py-2 text-xs text-gray-700 truncate max-w-[120px]">
                            ...
                            <div class="md:hidden divide-y divide-gray-100">
                                <div v-for="venta in grupo.ventas" :key="venta.IdVentas" class="p-2.5 hover:bg-gray-50 transition">
                                    <div class="flex justify-between items-start gap-2">
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span class="font-bold text-primary-700 text-xs">{{ venta.NumeroFactura || 'S/N' }}</span>
                                                <span class="inline-flex px-1.5 py-0.5 text-[8px] font-medium rounded-full bg-emerald-100 text-emerald-700">
                                                    {{ getEstadoAbrev(venta.IdEstado) }}
                                                </span>
                                            </div>
                                            ...
                                            <div class="text-[10px] text-gray-600 mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
                ...
                <!-- ==================== SIN DATOS ==================== -->
                <div v-if="!sucursalesConVentas.length || !props.ventas?.data?.length" class="bg-white rounded-xl shadow-sm p-10 text-center text-gray-400">
                    <i class="fas fa-inbox text-3xl mb-2 block"></i>
                    <p class="text-sm">No hay ventas que coincidan con los filtros</p>
                </div>

                <!-- ==================== PAGINACIÓN ==================== -->
                <div v-if="props.ventas?.data?.length" class="bg-white rounded-xl shadow-sm mt-3 px-3 py-2 border border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-2">
                    ...
                    <div class="flex gap-1 flex-wrap justify-center">
                        <button 
                            ...
                            :class="[
                                'px-2.5 py-0.5 rounded border text-[10px] transition',
                                link.active ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-200',
