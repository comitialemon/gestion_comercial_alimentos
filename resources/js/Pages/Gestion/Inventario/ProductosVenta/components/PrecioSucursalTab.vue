...
// 🔥 BÚSQUEDA DE SUCURSAL (igual que en mayoristas)
...
const eliminarFila = async (precio) => {
    ...
    try {
        const response = await axios.delete(`/gestion/inventario/productos-venta/precio-sucursal/${precio.IdPrecio}`)
        if (response.data.success) {
    ...
    <div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
            <div class="flex items-center gap-2">
                <i class="fas fa-store text-primary-500 text-[10px] sm:text-sm"></i>
                <span class="text-[10px] sm:text-xs font-medium text-gray-600">Precios diferenciados por sucursal</span>
            </div>
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-200" style="min-height: 300px;">
            ...
            <table class="min-w-full hidden md:table">
                <thead class="bg-primary-50">
                    <tr>
                        <th class="px-3 sm:px-4 py-2 text-left text-[10px] sm:text-[11px] font-semibold text-primary-700 uppercase tracking-wider">Sucursal</th>
                        <th class="px-3 sm:px-4 py-2 text-right text-[10px] sm:text-[11px] font-semibold text-primary-700 uppercase tracking-wider">Precio (Bs)</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <!-- Nueva fila desktop con buscador -->
                        ...
                        <td class="px-3 sm:px-4 py-2">
                            <div class="relative">
                                ...
                                <div 
                                    ...
                                    class="absolute z-[9999] mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg p-2 text-center text-gray-400 text-[10px] sm:text-xs"
                                >
                    ...
                    <tr v-for="precio in precios" :key="precio.IdPrecio" class="hover:bg-gray-50 transition">
                        <td class="px-3 sm:px-4 py-2 text-[10px] sm:text-sm text-gray-700">
                            <i class="fas fa-store text-primary-400 text-[8px] sm:text-[10px] mr-1 sm:mr-2"></i>
                            {{ nombreSucursal(precio.sucursal) }}
                        </td>
                        <td class="px-3 sm:px-4 py-2">
                            <div v-if="editandoId !== precio.IdPrecio" class="text-right font-semibold text-primary-600 text-[10px] sm:text-sm">
                            ...
                            <div v-else class="relative">
                                ...
                                <input 
                                    ...
                                    step="0.01" 
                                    min="0" 
                                    class="w-full pl-6 sm:pl-7 pr-2 py-1 border border-gray-300 rounded-md text-right text-[10px] sm:text-sm focus:ring-primary-500 focus:border-primary-500"
                                >
                            </div>
                        </td>
                        <td class="px-3 sm:px-4 py-2 text-center">
                    ...
                    <tr v-if="precios.length === 0 && !nuevaFila.editando">
                        <td colspan="3" class="px-3 sm:px-4 py-6 sm:py-8 text-center">
                            <i class="fas fa-store text-gray-300 text-xl sm:text-2xl mb-2 block"></i>
                            <p class="text-gray-400 text-[10px] sm:text-xs">No hay precios configurados</p>
            ...
            <div class="md:hidden divide-y divide-gray-100">
                ...
                <div v-for="precio in precios" :key="precio.IdPrecio" class="p-3 hover:bg-gray-50 transition">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                            <div v-if="editandoId !== precio.IdPrecio" class="font-semibold text-primary-600 text-xs">
                                {{ Number(precio.Precio).toFixed(2) }} Bs
                            </div>
                ...
                <!-- Nueva fila mobile con buscador -->
                ...
                <div v-if="precios.length === 0 && !nuevaFila.editando" class="p-6 text-center">
                    <i class="fas fa-store text-gray-300 text-xl mb-2 block"></i>
                    <p class="text-gray-400 text-[10px]">No hay precios configurados</p>
