...
const ejecutarEliminacion = async () => {
    ...
    try {
        const response = await axios.delete(`/gestion/inventario/productos-venta/precio-mayorista/${precioAEliminar.value.IdPrecioMayorista}`)
        if (response.data.success) {
            precios.value = precios.value.filter(p => p.IdPrecioMayorista !== precioAEliminar.value.IdPrecioMayorista)
            emit('update', precios.value)
        }
    } catch (error) {
    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <span class="text-xs sm:text-sm font-semibold text-gray-800 block">Precios Mayoristas</span>
                    <span class="text-[10px] sm:text-xs text-gray-500">Precios especiales para comisionistas por sucursal.</span>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden min-h-[300px]">
            ...
            <table class="min-w-full hidden md:table border-collapse">
                <thead class="bg-gray-50 border-b border-gray-100 sticky top-0">
                    <tr>
                        <th class="px-4 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider w-3/12">Sucursal</th>
                        <th class="px-4 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider w-5/12">Comisionista</th>
                        <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase tracking-wider w-2/12">Precio (Bs)</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-if="nuevaFila.editando" class="bg-primary-50/40">
                            ...
                            <div class="relative">
                                ...
                                <div v-if="showSucursalDropdown && busquedaSucursal && sucursalesFiltradas.length > 0" 
                                    ...
                                    <div v-for="s in sucursalesFiltradas" :key="s.id" @click="seleccionarSucursal(s)" @mousedown.prevent
                                        ...
                                        <span class="text-gray-700 truncate font-medium">{{ s.nombre }}</span>
                        ...
                        <td class="px-4 py-3 align-middle">
                            <div class="relative">
                                ...
                                >
                                <button v-if="busquedaIdentificador" @click="limpiarSeleccion" 
                                        class="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition">
                                    <i class="fas fa-times text-xs"></i>
                                </button>
                                
                                <div v-if="showDropdown && busquedaIdentificador && identificadoresFiltrados.length > 0" 
                                     ref="dropdownRef"
                                     class="absolute z-50 mt-1.5 w-full bg-white border border-gray-200 rounded-lg shadow-xl max-h-[220px] overflow-y-auto divide-y divide-gray-50">
                                    <div v-for="item in identificadoresFiltrados" :key="item.id || item.IdIdentificador" @click="seleccionarIdentificador(item)" @mousedown.prevent
                                         class="px-4 py-2.5 hover:bg-gray-50 cursor-pointer text-xs flex items-center gap-2.5 transition">
                                        <span class="font-mono text-[10px] font-bold bg-primary-50 px-2 py-0.5 rounded text-primary-700">{{ item.ci || item.CI_NIT }}</span>
                                        <span class="text-gray-700 truncate font-medium">{{ item.nombre || item.Nombre }}</span>
                                    </div>
                                </div>
                            </div>
                            <p v-if="errors.IdIdentificador" class="text-red-500 text-[11px] mt-1 flex items-center gap-1 font-medium"><i class="fas fa-exclamation-circle"></i> {{ errors.IdIdentificador }}</p>
                        </td>
                        <td class="px-4 py-3 align-middle">
                    ...
                    <tr v-for="precio in precios" :key="precio.IdPrecioMayorista" class="hover:bg-gray-50/80 transition-colors group">
                        <td class="px-4 py-3 text-xs text-gray-700 align-middle">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-gray-800">{{ precio.sucursal?.Nombre || '-' }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-xs text-gray-700 align-middle">
                            <div class="flex items-center gap-2">
                                <i class="fas fa-user text-gray-400 group-hover:text-primary-500 transition-colors"></i>
                                <span class="font-mono text-[10px] font-bold bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{{ precio.identificador?.CI_NIT || '-' }}</span>
                                <span class="font-medium text-gray-800 truncate">{{ precio.identificador?.Nombre || '-' }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 align-middle">
                            <div v-if="editandoId !== precio.IdPrecioMayorista" class="text-right font-bold text-primary-600 text-xs pr-3">
                        ...
                        <td class="px-4 py-3 text-center align-middle">
                            <div class="flex items-center justify-center gap-1.5">
                                <button v-if="editandoId !== precio.IdPrecioMayorista" @click="editarFila(precio)" class="text-blue-600 hover:text-blue-700 p-1.5 hover:bg-blue-50 rounded-md transition" title="Editar">
                                    <i class="fas fa-edit text-xs"></i>
                                </button>
                                <button v-if="editandoId === precio.IdPrecioMayorista" @click="guardarEdicion(precio)" :disabled="loading" class="text-emerald-600 hover:text-emerald-700 p-1.5 hover:bg-emerald-50 rounded-md transition" title="Guardar">
                                    <i class="fas fa-save text-xs"></i>
                                </button>
                                <button v-if="editandoId === precio.IdPrecioMayorista" @click="cancelarEdicion" class="text-gray-400 hover:text-gray-500 p-1.5 hover:bg-gray-50 rounded-md transition" title="Cancelar">
                                    <i class="fas fa-times text-xs"></i>
                                </button>
                                <button v-if="editandoId !== precio.IdPrecioMayorista" @click="confirmarEliminarFila(precio)" class="text-red-500 hover:text-red-600 p-1.5 hover:bg-red-50 rounded-md transition" title="Eliminar">
                    ...
                    <tr v-if="precios.length === 0 && !nuevaFila.editando">
                        <td colspan="4" class="px-4 py-12 text-center">
                            <div class="max-w-[240px] mx-auto">
                                ...
                                <p class="text-gray-400 text-xs font-medium">No hay precios mayoristas configurados para este producto.</p>
                                <button @click="agregarFila" class="mt-2 text-primary-600 hover:text-primary-700 text-xs font-semibold underline decoration-2 underline-offset-4">+ Agregar precio mayorista</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <div class="md:hidden divide-y divide-gray-100">
                <div v-for="precio in precios" :key="precio.IdPrecioMayorista" class="p-4 hover:bg-gray-50/50 transition-colors">
                    <div class="flex justify-between items-start gap-4">
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-1.5 mb-1 flex-wrap">
                                <i class="fas fa-store text-primary-400 text-[10px]"></i>
                                <span class="text-xs font-semibold text-gray-800 truncate">{{ precio.sucursal?.Nombre || '-' }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <i class="fas fa-user text-primary-400 text-[10px]"></i>
                                <span class="font-mono text-[9px] font-bold bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{{ precio.identificador?.CI_NIT || '-' }}</span>
                                <span class="text-[11px] text-gray-600 truncate">{{ precio.identificador?.Nombre || '-' }}</span>
                            </div>
                        </div>
                        <div class="flex-shrink-0 text-right">
                            <span class="text-[10px] text-gray-400 block mb-0.5 uppercase tracking-wider font-bold">Precio</span>
                            <div v-if="editandoId !== precio.IdPrecioMayorista" class="font-bold text-primary-600 text-xs">
                                {{ Number(precio.Precio).toFixed(2) }} Bs
                            </div>
                ...
                <div v-if="nuevaFila.editando" class="p-4 bg-primary-50/30 space-y-3">
                    ...
                    <div>
                        <label class="text-[11px] font-bold text-gray-500 uppercase tracking-wider block mb-1">Comisionista</label>
                        <div class="relative">
                            <input 
                                ref="inputRef"
                                type="text" 
                                ...
                                placeholder="Buscar por CI o nombre..."
                            >
                            <button v-if="busquedaIdentificador" @click="limpiarSeleccion" class="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400">
                                <i class="fas fa-times text-xs"></i>
                            </button>
                            ...
                            <div v-if="showDropdown && busquedaIdentificador && identificadoresFiltrados.length > 0"
                                ...
                                <div v-for="item in identificadoresFiltrados" :key="item.id || item.IdIdentificador" @click="seleccionarIdentificador(item)" @mousedown.prevent
                                     class="px-3 py-2 hover:bg-gray-50 text-xs flex items-center gap-2 border-b border-gray-50 last:border-0">
                                    <span class="font-mono text-[9px] font-bold bg-primary-50 px-1.5 py-0.5 rounded text-primary-700">{{ item.ci || item.CI_NIT }}</span>
                                    <span class="text-gray-700 truncate font-medium">{{ item.nombre || item.Nombre }}</span>
                                </div>
                            </div>
                        </div>
                        <p v-if="errors.IdIdentificador" class="text-red-500 text-[11px] mt-1 font-medium"><i class="fas fa-exclamation-circle"></i> {{ errors.IdIdentificador }}</p>
                    </div>
                ...
                <div v-if="precios.length === 0 && !nuevaFila.editando" class="p-8 text-center">
                    ...
                    <p class="text-gray-400 text-xs font-medium">No hay precios mayoristas configurados</p>
                    <button @click="agregarFila" class="mt-1.5 text-primary-600 text-xs font-semibold underline">+ Agregar precio mayorista</button>
                </div>
            </div>
        </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-[99999] flex items-center justify-center p-4">
            ...
            <div class="bg-white rounded-xl shadow-xl overflow-hidden max-w-md w-full border border-gray-100 transform transition-all z-10 p-5 space-y-4">
                <div class="flex items-start gap-3.5">
                    ...
                    <div class="space-y-1">
                        <h3 class="text-sm font-bold text-gray-900">¿Eliminar precio mayorista?</h3>
                        <p class="text-xs text-gray-500 leading-relaxed">
                            ¿Está seguro de que desea eliminar el precio mayorista para 
                            <span class="font-bold text-gray-700">"{{ precioAEliminar?.identificador?.Nombre }}"</span>
                            en la sucursal 
                            <span class="font-bold text-gray-700">"{{ precioAEliminar?.sucursal?.Nombre }}"</span>?
                            Esta acción eliminará el precio especial para este comisionista.
                        </p>
                    </div>
                </div>
