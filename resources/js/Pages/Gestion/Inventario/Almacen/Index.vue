...
const formData = ref({ Almacen: '', AlmacenPrincipal: 0 })
const errors = ref({})

const resetForm = () => {
    editando.value = false
    editId.value = null
    formData.value = { Almacen: '', AlmacenPrincipal: 0 }
}
...
const guardar = () => {
    if (editando.value) {
        router.put(`/gestion/inventario/almacen/${editId.value}`, formData.value, {
            preserveScroll: true,
            onSuccess: () => resetForm(),
            onError: (err) => { errors.value = err }
        })
    } else {
        router.post('/gestion/inventario/almacen', formData.value, {
            preserveScroll: true,
            onSuccess: () => resetForm(),
            onError: (err) => { errors.value = err }
        })
            ...
            <div class="max-w-4xl mx-auto">
                <div class="text-center mb-6">
                    <div class="inline-flex items-center justify-center w-14 h-14 bg-indigo-100 rounded-2xl mb-3">
                        <i class="fas fa-warehouse text-xl text-indigo-600"></i>
                    </div>
                    <h1 class="text-xl font-bold text-gray-900">Almacenes</h1>
                    <p class="text-xs text-gray-500">Administra los almacenes de la sucursal</p>
                </div>

                <div class="bg-white rounded-xl shadow-sm p-4 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <input type="text" v-model="formData.Almacen" placeholder="Nombre del almacén" class="w-full border rounded-lg px-3 py-2 text-sm" :class="{ 'border-red-500': errors.Almacen }" />
                        ...
                        <div class="flex gap-2">
                            <label class="flex items-center gap-2 px-3 py-2 border rounded-lg">
                                <input type="checkbox" v-model="formData.AlmacenPrincipal" :true-value="1" :false-value="0" class="rounded" />
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item in items.data" :key="item.IdAlmacen" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.IdAlmacen }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ item.Almacen }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="item.AlmacenPrincipal === 1" class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">
                                        <i class="fas fa-check mr-1"></i> Principal
                                    </span>
                                    <span v-else class="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">Secundario</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button @click="editar(item)" class="text-indigo-600 hover:text-indigo-900 mr-3"><i class="fas fa-edit"></i></button>
                                    <button @click="eliminar(item.IdAlmacen, item.Almacen)" class="text-red-600 hover:text-red-900"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                            <tr v-if="items.data.length === 0"><td colspan="4" class="px-6 py-12 text-center text-gray-500">No hay almacenes registrados</td></tr>
                        </tbody>
                    </table>
