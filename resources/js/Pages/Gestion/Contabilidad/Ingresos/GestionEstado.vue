...
// 🔥 Toggle manual (sin checkbox nativo)
const toggleSwitch = (ingreso) => {
    if (cambiando.value[ingreso.IdIngreso]) return
    const nuevoEstado = ingreso.ActivoInactivo === 1 ? 0 : 1
            ...
            <div class="max-w-full lg:max-w-7xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-500 hidden xs:block">Activar o desactivar comprobantes de ingreso</p>
                        </div>
                    </div>
                ...
                <div v-if="isMobile" class="space-y-3">
                    <div v-for="ingreso in ingresos.data" :key="ingreso.IdIngreso" class="bg-white rounded-lg shadow-sm p-3">
                        ...
                        <div class="space-y-2 text-xs">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium truncate max-w-[180px]">{{ ingreso.identificador?.Nombre || '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Glosa:</span>
                                <span class="font-medium truncate max-w-[180px]">{{ ingreso.Glosa || '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Monto:</span>
                                <span class="font-bold text-blue-600">{{ formatearMonto(ingreso.TotalBolivianos) }} Bs</span>
                            </div>
                            <div class="flex justify-between items-center pt-1 border-t mt-1">
                                ...
                                <div class="relative inline-flex items-center cursor-pointer" @click="toggleSwitch(ingreso)">
                                    <div class="w-9 h-5 rounded-full transition-colors duration-200 ease-in-out"
                                        :class="ingreso.ActivoInactivo === 1 ? 'bg-blue-600' : 'bg-gray-300'">
                                    ...
                                    <span class="ml-2 text-[10px]" :class="cambiando[ingreso.IdIngreso] ? 'text-gray-400' : (ingreso.ActivoInactivo === 1 ? 'text-green-600' : 'text-gray-500')">
                                        <i v-if="cambiando[ingreso.IdIngreso]" class="fas fa-spinner fa-spin"></i>
                                        <span v-else>{{ ingreso.ActivoInactivo === 1 ? 'Activo' : 'Inactivo' }}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div v-if="ingresos.data?.length === 0" class="bg-white rounded-lg shadow-sm p-8 text-center">
                        ...
                        <p class="text-xs text-gray-400">No hay comprobantes de ingreso</p>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-blue-50">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-blue-700 uppercase">N° Ingreso</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-blue-700 uppercase">Fecha</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-blue-700 uppercase">Recibido de</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-blue-700 uppercase">Glosa</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="ingreso in ingresos.data" :key="ingreso.IdIngreso" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-2 text-xs text-gray-700 max-w-[150px] truncate">{{ ingreso.identificador?.Nombre || '-' }}</td>
                                    <td class="px-3 py-2 text-xs text-gray-600 max-w-[200px] truncate">{{ ingreso.Glosa || '-' }}</td>
                                    <td class="px-3 py-2 text-xs text-right font-semibold text-blue-600">{{ formatearMonto(ingreso.TotalBolivianos) }} Bs</td>
                    ...
                    <div v-if="ingresos.links && ingresos.links.length > 1" class="px-3 py-2 border-t border-gray-200 bg-gray-50">
                        <div class="flex justify-between items-center text-xs">
                            ...
                            <div class="flex gap-0.5 flex-wrap">
                                <Link v-for="link in ingresos.links" :key="link.label" :href="link.url || '#'" class="px-2 py-0.5 rounded border text-xs" :class="{ 'bg-blue-600 text-white border-blue-600': link.active, 'bg-white text-gray-700 hover:bg-gray-50': !link.active && link.url, 'opacity-50 cursor-not-allowed': !link.url }" v-html="link.label" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="isMobile && ingresos.links && ingresos.links.length > 1" class="mt-3 bg-white rounded-lg shadow-sm p-2">
                    <div class="flex justify-center gap-0.5 flex-wrap">
                        <Link v-for="link in ingresos.links" :key="link.label" :href="link.url || '#'" class="px-2 py-1 rounded border text-xs min-w-[32px] text-center" :class="{ 'bg-blue-600 text-white border-blue-600': link.active, 'bg-white text-gray-700 hover:bg-gray-50': !link.active && link.url, 'opacity-50 cursor-not-allowed': !link.url }" v-html="link.label" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl w-full max-w-[90%] sm:max-w-sm overflow-hidden shadow-xl">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            <h3 class="font-bold text-gray-800 text-sm sm:text-base">
                                {{ modalData.accion === 'activar' ? 'Activar Ingreso' : 'Desactivar Ingreso' }}
                            </h3>
                ...
                <div class="p-4 sm:p-5">
                    ...
                    <p class="text-[10px] sm:text-xs text-gray-400 text-center mt-2">
                        {{ modalData.accion === 'activar' ? 'Al activarlo, el ingreso se marcará como contabilizado.' : 'Al desactivarlo, el ingreso volverá a estado borrador y podrá editarse.' }}
                    </p>
                </div>
...
<style scoped>
@media (max-width: 640px) { .xs\:inline { display: inline; } .xs\:block { display: block; } }
</style>