            ...
            <div class="max-w-full mx-auto">
                ...
                <div class="flex justify-between items-center mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 bg-guindo-100 rounded-lg flex items-center justify-center">
                            <i class="fas fa-book-open text-guindo-600 text-base"></i>
                        </div>
                        <div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-guindo-50">
                                <tr>
                                    <th class="px-4 py-2 text-left text-xs font-semibold text-guindo-700 uppercase">N° Diario</th>
                                    <th class="px-4 py-2 text-left text-xs font-semibold text-guindo-700 uppercase">Fecha</th>
                                    <th class="px-4 py-2 text-left text-xs font-semibold text-guindo-700 uppercase">Tipo Diario</th>
                                    <th class="px-4 py-2 text-left text-xs font-semibold text-guindo-700 uppercase">Sucursal</th>
                                    <th class="px-4 py-2 text-left text-xs font-semibold text-guindo-700 uppercase">Operador</th>
                                    <th class="px-4 py-2 text-center text-xs font-semibold text-guindo-700 uppercase">Estado</th>
                                    <th class="px-4 py-2 text-center text-xs font-semibold text-guindo-700 uppercase">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="diario in diarios.data" :key="diario.IdDiario" class="hover:bg-gray-50 transition">
                                    <td class="px-4 py-2 text-sm font-mono text-guindo-700 font-semibold">{{ diario.NumeroDiario }}</td>
                    ...
                    <div v-if="diarios.links && diarios.links.length > 1" class="px-4 py-3 border-t border-gray-200">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex gap-1">
                                <Link v-for="link in diarios.links" :key="link.label" :href="link.url || '#'" class="px-3 py-1 rounded border text-sm" :class="{ 'bg-guindo-600 text-white border-guindo-600': link.active, 'bg-white text-gray-700 hover:bg-gray-50': !link.active && link.url, 'opacity-50 cursor-not-allowed': !link.url }" v-html="link.label" />
                            </div>
                        </div>
                    </div>
                </div>
