...
const cargarCatalogos = async () => {
    ...
    try {
        ...
        // 🔥 Monedas (filtradas desde la API)
        const monedasRes = await axios.get('/api/catalogos/monedas')
        monedas.value = monedasRes.data || []
        console.log('Monedas filtradas:', monedas.value)
        
        // Métodos de pago
        const metodosRes = await axios.get('/api/catalogos/metodos-pago')
        ...
        console.log('Métodos de pago:', metodosPago.value)
...
const verificarNit = async () => {
    ...
    try {
        // 🔥 LLAMA A TU API DE GESTIÓN
        const response = await axios.post('/venta-factura/buscar-cliente', { nit: nitCliente.value })
...
const volverALaVenta = () => {
    if (confirm('¿Volver a la venta? Los datos de pago no se guardarán.')) {
        window.location.href = '/venta-factura/nueva'
    }
}
...
const procesarPago = async () => {
    if (!pagoCorrecto) {
        alert('El monto total debe ser igual a la deuda')
        return
    }
    
    if (totalRegistrado.value !== totalConDescuento.value) {
        alert(`Total registrado (${totalRegistrado.value.toFixed(2)} Bs) no coincide con total a pagar (${totalConDescuento.value.toFixed(2)} Bs)`)
        return
    }
    ...
    router.post('/venta-factura/procesar-pago', {
        venta_id: props.venta.IdVentas,
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div class="mb-3">
                    <button @click="volverALaVenta" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300">
                        <i class="fas fa-arrow-left text-xs"></i> Volver a la Venta
                    </button>
                </div>
